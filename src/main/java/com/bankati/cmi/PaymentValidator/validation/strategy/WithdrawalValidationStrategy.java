...
@Component
public class WithdrawalValidationStrategy implements PaymentValidationStrategy {
    @Override
    public void validatePayment(PaymentRequest request) {
        if (request.getSourceAccount() == null) {
            throw new IllegalArgumentException("Le compte source est obligatoire pour un retrait.");
        }
        ...
        if (request.getTargetAccount() != null ) {
            throw new IllegalArgumentException("Le compte cible est interdit pour un retrait.");
        }
    }
}
